...
async function loadPageSections(params = {}) {
  try {
    ...
    // 🛡️ 增加 Cloudflare 拦截检测
    if (htmlContent.includes("Just a moment") || htmlContent.includes("Cloudflare")) {
        throw new Error("被安全盾拦截，请开启全局代理或稍后再试");
    }
...
async function parseHtml(htmlContent) {
  ...
  for (const sectionElement of sectionElements) {
    ...
    const sectionTitleText = sectionTitle.length > 0 ? sectionTitle.text().trim() : "列表内容";
    ...
    if (itemElements && itemElements.length > 0) {
      for (const itemElement of itemElements) {
        ...
        const url = titleId.length > 0 ? titleId.attr("href") : "";
        ...
        if (url && url.includes("jable.tv")) {
          ...
          const cover = coverId.length > 0 ? (coverId.attr("data-src") || coverId.attr("src") || "") : "";
          const video = coverId.length > 0 ? (coverId.attr("data-preview") || "") : "";
          const title = titleId.length > 0 ? titleId.text().trim() : "未知标题";
          const duration = durationId.length > 0 ? durationId.text().trim() : "";
...
async function loadDetail(link) {
  ...
  if (response.data && (response.data.includes("Just a moment") || response.data.includes("Cloudflare"))) {
      throw new Error("视频详情被安全盾拦截，请开启全局代理或稍后再试");
  }
